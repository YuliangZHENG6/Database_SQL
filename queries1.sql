...
Changes made from previous submission:
Currency is kept in a new column. Hence, we can filter out the $­sign.
...
WHERE TT.TAG_ID = TAGS.TAG_ID AND PC.TITLE_ID = TT.TITLE_ID AND PC.PUBLICATION_ID = PA.PUBLICATION_ID­­CONNECT TABLES TGT AND A.AUTHOR_ID = PA.AUTHOR_ID ­­AND P.PUBLICATION_ID = PA.PUBLICATION_ID ­­CONNECT TABLES TGT
...
FROM TITLE T, TITLE_AWARDS TA, REVIEWS R ­­ID=ID MAY LOSE SOME DATA
WHERE T.TITLE_ID = TA.TITLE_ID AND T.TITLE_ID = R.TITLE_ID GROUP BY T.TITLE
ORDER BY POPULARITY DESC)
WHERE ROWNUM <= 3; ­­WRTING ‘=’ DOESNT WORK
The following code is the correct method. However, no optimisation is done so it will be very slow.
­­NOT SURE IF ITS BETTER­­CORRECT WAY BUT MUCH SLOWER:
SELECT TITLE FROM
(SELECT TITLE.TITLE, COUNT(TA.AWARD_ID)+COUNT(R.REVIEW_ID) AS
POPULARITY
FROM TITLE
LEFT JOIN TITLE_AWARDS TA
ON TITLE.TITLE_ID = TA.TITLE_ID
LEFT JOIN REVIEWS R
ON TITLE.TITLE_ID = R.TITLE_ID
GROUP BY TITLE.TITLE ­­GROUP BY MUST ALWAYS LINK TO THE SELECT ORDER BY POPULARITY DESC)
WHERE ROWNUM <= 3;
